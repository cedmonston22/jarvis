...
separate RAF loop in src/hooks/useGestures.ts, per frame:
  1. step per-hand airTap click detector
...
- **Click has one detector, 2D-only.** `airTap.ts` watches a V-shape in `fingerStraightness2D` (finger-curl click). 2D is deliberate — MediaPipe's z is noisy; the joint-angle signal is also immune to the tilts-as-taps failure mode that killed the original z-velocity detector. A forwardTap (bbox-growth, hand-toward-camera) detector was tried and removed: the "reach toward UI to hover" motion naturally triggered it, producing ghost clicks on hover.
...
- **PinchTarget: pinch-and-release over a target = click too.** When a pinch on a target releases without ever promoting to a drag, PinchTarget fires `onClick`. Gives pinch as a second click modality alongside the curl detector.
- **Passive poses short-circuit everything.** `isFist(hand)` and `isOpenHand(hand)` in `detectors/poses.ts` classify the explicit dead-end poses: fist (all four non-thumb fingers have 2D cosine < 0.1; reach is NOT used because side-view fists have nearly-flat z, so 3D reach collapses to 2D reach which stays high for curled fingers projecting outward) and wave (all four clearly extended — reach > 1.85 AND 2D cos > 0.88). Threshold of 0.1 sits between the highest observed fist reading (-0.06 on a side-view fist's pinky) and the lowest tri-pinch reading (+0.14 on backlit-tri-pinch-front-facing's index). When either fires on a hand, `useGestures` forces that hand's click detector into cooldown, resets its handPinch state to non-pinching (emitting clean end events if transitioning mid-pinch), and — for the primary hand — ORs `passivePose !== 'none'` into `suppressPinch` so the state machine can't enter PINCH_PENDING. `suppressPinch` only blocks IDLE → PINCH_PENDING, so in-flight drags still complete normally. The gate does not affect pointer emission; `isPointerPose` already rejects both poses by itself, so a passive hand simply produces no events.
- **`isPointerPose` uses 2D cosine AND 3D reach.** Index: `fingerStraightness2D` > 0.3 (loose, so bent-while-pinching still reads as pointing). A non-index finger counts as "extended" only when BOTH `fingerStraightness2D > 0.88` AND `fingerReach > 1.85`. The AND gate fixes front-facing pointer flicker: foreshortened neighbor fingers produce noisy 2D cosines that can spuriously read above 0.88, falsely rejecting pointing. `fingerReach` (MCP→TIP distance / MCP→PIP segment length) is orientation-invariant — curled ≤ 1.55, extended ≥ 2.15 across every fixture — so the reach check acts as a sanity gate that the 2D noise can't flip.
...
- `src/gestures/` — pure functions, no DOM, no React, no side effects. Unit-tested. Includes `bus.ts`, `BusContext.tsx`, `useGestureBus.ts`, `stateMachine.ts`, `fingers.ts` (`pinchDistance`, `triPinchDistance`, straightness + `fingerReach` helpers), `handTopology.ts`, `oneEuro.ts`, `types.ts`, and `detectors/{pointer.ts, poses.ts, airTap.ts, handPinch.ts, spread.ts}`. `poses.ts` owns `isFist` / `isOpenHand` — the passive-pose gate. `handPinch.ts` tracks both regular pinch and tri-pinch state per hand.
...
- Don't revive a "hand toward camera" click detector. Both the z-velocity version (scrapped for tilts-as-taps) and the bbox-growth `forwardTap.ts` version (scrapped for reach-to-hover ghost clicks) have been tried and removed. Clicks are curl-only (`airTap.ts`) plus PinchTarget pinch-and-release.
...
- Don't add a `fingerReach` / "palm-length" gate to PINCH detection. It was tried to reject claw poses but also rejected firm deliberate pinches where the index curls inward. Pinch rejection for pathological poses relies on `pinchDistance`'s own ratio math, not a reach heuristic. (The `fingerReach` helper itself exists and is used by `isPointerPose` as an AND gate with 2D straightness — that's a different detector with a different failure mode.)
